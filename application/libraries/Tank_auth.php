...
class Tank_auth
{
	...
	function login($login, $password, $login_by_username, $login_by_email, $level_check=0)
	{
		...
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok without password check
	 ...
	 * username, email, @password, @fname, @loc ,new_email_key (if any).
	 *
	 * @param	string
	 * @param	string
	 * @param	string
	 * @param	string	 
	 */
	function create_user($username, $email, $password,  $d_token, $fname="", $email_activation=true)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');
                        $this->error = array('message' => 'User name is already in use');

		} elseif (!$this->ci->users->is_email_available($email)) {
			$this->error = array('email' => 'auth_email_in_use');
                        $this->error = array('message' => 'emial id is already in use');

		} 
                elseif (!$this->ci->users->is_fname_available($fname)) {
			$this->error = array('fname' => 'auth_facebook_name_in_use');
                        $this->error = array('message' => 'Facebook name is already in use');
